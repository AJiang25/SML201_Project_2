...
# get the range of time given pdt
# getTimeRanges <- function(pdt) {
#   time_ranges <- matrix(nrow = length(pdt), ncol = 2)
#   for (i in seq_along(pdt)) {  
#     if (pdt[i] == 1) {
#       time_ranges[i, ] <- c(0, 759)
#     } else if (pdt[i] == 2) {
#       time_ranges[i, ] <- c(800, 1559)
#     } else {
#       time_ranges[i, ] <- c(1600, 2359)
#     }
#   }
#   
#   return(time_ranges)  
# }
getTimeRange <- function(pdt) {
  if (pdt[i] == 1) {
      return(c(0, 759))
    } else if (pdt[i] == 2) {
      return(c(800, 1559))
...
# QUESTIONS: in the case that we have multiple qualifying non-stop flights, which one do we choose? (arrange by arr date? or flight duration?)
# QUESTIONS: when we are optimizing the flight dates (one day before match_dt), does this mean departure date or arrival date
findDirectFlights = function(dep_airport, arr_airport, match_dow, prev_dt, match_dt, pdt) {
  # get time ranges based on preferred departure time
  ...
  directFlightOptions <- wc_flights |>
    ...
    filter(
      ...
      # (dep >= time_ranges[1, 1] & dep <= time_ranges[1, 2]) | 
      # (dep >= time_ranges[2, 1] & dep <= time_ranges[2, 2])
...
# QUESTION: what priority do we select our flight? layover? total_dist? arrival date?
findOneConnection = function(dep_airport, arr_airport, match_dow, prev_dt, match_dt, pdt) {
  ...
  qualifyingConnections <- qualifyingFirstLeg |>
    ...
    slice(1) |>
    ungroup() |>
    arrange(total_dist, desc(final_arr_date), layover_duration) 
...
findTwoConnection = function(dep_airport, arr_airport, match_dow, prev_dt, match_dt, pdt) {
  ...
  qualifyingConns <- qualifyingFirstLeg |>
    ...
    slice(1) |>
    ungroup() |>
    arrange(desc(final_arr_date), total_layover_duration) 
    ...
    for (i in 1:(nrow(countryInfo) - 1)) {
        ...
        if (conn[i] == 0) {
            ...
            itin <- rbind(itin, data.frame(
                ...
                dod = flight$dep_date, tod = flight$dep,
                doa = flight$arr_date, toa = flight$arr,
                dist = flight$dist))
        } else if (conn[i] == 1) {
            ...
            itin <- rbind(itin, data.frame(
                ...
                dod = flight$first_dep_date, tod = flight$first_dep,
                doa = flight$first_arr_date, toa = flight$first_arr,
                dist = flight$first_dist))
            itin <- rbind(itin, data.frame(
              flt = flt_number, leg = 2,
              dep = flight$first_dest, arr = flight$second_dest,
              dod = flight$final_dep_date, tod = flight$second_dep,
              doa = flight$final_arr_date, toa = flight$second_arr,
              dist = flight$second_dist
            ))
        } else {
            ...
            itin <- rbind(itin, data.frame(
              flt = flt_number, leg = 2,
              dep = flight$first_dest, arr = flight$second_dest,
              dod = flight$second_dep_date, tod = flight$second_dep,
              doa = flight$second_arr_date, toa = flight$second_arr,
              dist = flight$second_dist
            ))
            itin <- rbind(itin, data.frame(
              flt = flt_number, leg = 3,
              dep = flight$second_dest, arr = flight$third_dest,
              dod = flight$final_dep_date, tod = flight$third_dep,
              doa = flight$final_arr_date, toa = flight$third_arr,
              dist = flight$third_dist
            ))
...
our_itinerary |> 
  gt() |>
  tab_header(title = "Itinerary to Follow Argentina in 2026 World Cup") |>
  cols_label(
    flt = "Flight No.",
    leg = "Leg No.",
    dep = "Depature Airport",
    arr = "Arrival Airport",
    dod = "Date of Departure",
    tod = "Time of Depature",
    doa = "Date of Arrival",
    toa = "Time of Arrival",
    dist = "Distance"
  ) |>
...
# find a golf course near Boston since we have free time on the 22nd of june in Boston to play a round of golf
countryInfo <- wc_sched |>
  filter(ctry_a == our_country | ctry_b == our_country) |>
  arrange(match_dt) |>
  select(ctry_a, ctry_b, match_dt, venue, city, lat, lng)

latLng <- countryInfo |>
  filter(venue == "Gillette Stadium") |>
  select(lat, lng)


findClosestGolfCourse <- function(latLng) {
  golfInfo <- wc_golf |>
    mutate(dist = dist_mi(latLng$lat, latLng$lng, lat, lng)) |>
    arrange(dist) |>
    slice(1)
}

golfCourse <- findClosestGolfCourse(latLng)

golf_round <- list(
  course = golfCourse$course,
  tee_time = as.POSIXct("2026-06-22 15:00:00"),
  dist = golfCourse$dist,
  ...
  fun_fact = "It was, in fact, the very first country club in the U.S., dating to 1882."
)
...
golf_popup <- paste0(
  "Course: ", golfCourse$course, "<br>",
  "Country: ", golfCourse$country, "<br>",
  "Latitude: ", golfCourse$lat, "<br>",
  "Longitude: ", golfCourse$lng, "<br>",
...
leaflet() |>
  ...
  addAwesomeMarkers(data = countryInfo, ~lng, ~lat,
                    label = ~match_num,
                    popup = ~popup) |>
  addMarkers(data = golfCourse, ~lng, ~lat, 
             icon = golf_icon,
