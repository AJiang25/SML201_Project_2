...
# wc_shots = wc_shots %>% 
#   mutate(regression_data = exp(predict(model2)))
...
#   hc_add_series(wc_shots, hcaes(x = d, y = regression_data), type = 'line')

wc_shots <- wc_shots %>%
  mutate(regression_data = exp(predict(model2)))

ggplot(wc_shots, aes(x = d, y = xg)) +
  geom_point(alpha = 2) +
  geom_line(aes(y = regression_data), color = "blue", size = 1) +
  labs(
    title = "Expected Goals vs Distance",
...
game_model = function(a, b) {
    ...
    while (total_time < 90 * 60) { # 90 minutes
        ...
        posession_time = max(5, rnorm(1, mean = 45 * 60, sd = 13 * 60)) 

        # Check if the first half ends
        if (total_time < 45 * 60 && total_time + possession_time > 45 * 60) {
            possession = "b" # Switch possession to country b at the start of the second half
            total_time = 45 * 60 # End the first half
            next
        }

        total_time = total_time + posession_time 
...
wc_grpstage = wc_sched %>%
    mutate(
        gls = map2(ctry_a, ctry_b, ~ game_mode(.x, .y)),
        gls_a = map_int(gls, 1),
        gls_b = map_int(gls, 2)
    ) %>%
    select(match, group, ctry_1, ctry_b, gls_a, gls_b)
```
