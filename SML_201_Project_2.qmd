...
our_seed = sum(920327399, 999999999, 920400588) %% 1746
```
...
itinerary = function(ctry, pdt, conn) {
  #3 group matches for the country
  matches = wc_sched %>% 
    filter(ctry_a == ctry | ctry_b == ctry) %>% 
    arrange(match_dt) %>% 
    slice(1:3)
  
  #nearest airports to each of the venues where the matches will be played
}
...
wc_corr = wc_shots %>% 
  select(pos_d, pos_f, pos_m, xg, x, y, d, theta)
wc_corr = cor(wc_corr)
